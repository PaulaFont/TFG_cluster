...
#JSON
def parse_gexf_to_json(gexf_file_path):
    """Parse GEXF file and convert to JSON format for JavaScript visualization"""
    try:
        # Load the GEXF file
        G = nx.read_gexf(gexf_file_path)
        
        # Convert to JSON format suitable for JavaScript libraries
        nodes = []
        edges = []
...
def create_graph_html(graph_data):
    """Create HTML with embedded JavaScript for interactive graph visualization"""
    
    # Convert Python dict to JSON string for JavaScript
    graph_json = json.dumps(graph_data)
    
    html_template = f"""
    <div id="graph-container-{hash(str(graph_data)) % 10000}" style="width: 100%; height: 500px; border: 1px solid #ccc; background-color: white;"></div>
    
    <script src="https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.5/d3.min.js"></script>
    <script>
    // Use unique container ID to avoid conflicts
    const containerId = "graph-container-{hash(str(graph_data)) % 10000}";
    ...
    function initGraph() {{
        ...
        if (graphData.error) {{
            document.getElementById('graph-container').innerHTML = 
                '<div style="padding: 20px; color: red;">Error: ' + graphData.error + '</div>';
            return;
        }}
        
        if (graphData.nodes.length === 0) {{
            document.getElementById('graph-container').innerHTML = 
                '<div style="padding: 20px; color: gray;">No nodes found in graph</div>';
            return;
        }}
        ...
        const width = Math.max(containerRect.width - 20, 600);  // Leave some padding
        const height = Math.max(containerRect.height - 20, 400);
        ...
        node.on("mouseover", function(event, d) {{
            d3.select(this).attr("r", (d.size || 8) + 4);
        }})
        .on("mouseout", function(event, d) {{
            d3.select(this).attr("r", (d.size || 8) + 2);
        }});
...
def update_graph(gexf_filename):
    """Function called when graph needs to be updated"""
    if gexf_filename is None:
        return "<div>No graph found</div>"
    graph_data = parse_gexf_to_json(gexf_filename)

    return create_graph_html(graph_data)
...
def visualize_knowledge_graph(
    ...
    try:
        ...
        elif output_format == "html_json":
            temp_filepath = os.path.join(output_directory, f"{filename_prefix}.gexf")
            nx.write_gexf(graph, temp_filepath)
            html_text = update_graph(temp_filepath)

            with open(output_filepath, "w", encoding="utf-8") as file:
                file.write(html_text)
            print(f"Graph saved in gexf format to: {output_filepath}. Using JSON")
